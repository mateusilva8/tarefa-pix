...
export class MovimentacaoController {
  ...
  async create(req: Request, res: Response): Promise<Response> {
    ...
    if (body.tipo == "E" && cdItemQtdade == true) {
  ...
  async delete(req: Request, res: Response): Promise<Response> {
    ...
    if (movimentacao.qtde <= movimentacao.cd_item.qtdade) {
      movimentacao.situacao = "I";
      movimentacao.cd_item.qtdade =
        movimentacao.cd_item.qtdade - movimentacao.qtde;
      await movimentacao.save();
      return res
        .status(200)
        .json(
          `Movimentação ID# ${movimentacao.id} inativada com sucesso!${movimentacao.cd_item.qtdade}`
        );
    } else {
      return res
        .status(401)
        .json({ mensagem: "A entrega do item já foi feita" });
    }
  }
}
