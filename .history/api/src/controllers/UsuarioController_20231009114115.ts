...
export class UsuarioController {
  ...
  async loginTeste(req: Request, res: Response): Promise<Response> {
    ...
    if (!usuario) {
      return res
        .status(401)
        .json({ mensagem: "Usuário ou senha não encontrados" });
    }
  ...
  async login(req: Request, res: Response): Promise<Response> {
    ...
    if (loginOk) {
      ...
      if (match) {
        ...
        return res.status(200).json({ mensagem: "Autenticado com sucesso" });
      } else {
