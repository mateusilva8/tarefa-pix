...
export class PessoaBeneficiarioController {
  ...
  async find (req: Request, res: Response): Promise<Response>  {
    ...
    let beneficiario : PessoaBeneficiario | null = await PessoaBeneficiario.findOneBy({id});
    if (!beneficiario) {
  ...
  async delete (req: Request, res: Response): Promise<Response> {
    ...
    return res.status(200).json(`Beneficiário ID# ${beneficiario.id} inativado com sucesso!`);
  }
}
